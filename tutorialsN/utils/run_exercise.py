...
def execute_and_terminate_script(switch_pids, duration):
    ...
    for pid in switch_pids:
        process = subprocess.Popen(["/home/matheus/tutorialsN/utils/cpu.ssh", pid])
...
class TrafficGenerator(Thread):
    ...
    def run(self):
        ...
        while time.time() - start_time < self.duration:
            # Start iperf clients on h1, h2, and h3 with specified bandwidth limits
            h1.cmd(f'iperf -c {h2.IP()} -t 10 -b 1M &')   # 1 MB/s from h1 to h2
            h2.cmd(f'iperf -c {h3.IP()} -t 10 -b 10M &')  # 10 MB/s from h2 to h3
            h1.cmd(f'iperf -c {h3.IP()} -t 10 -b 10M &')  # 10 MB/s from h1 to h3
            h2.cmd(f'iperf -c {h1.IP()} -t 10 -b 1M &')   # 1 MB/s from h2 to h1
            time.sleep(10)
